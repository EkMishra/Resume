...
export class LandingPageComponent {
  ...
  //   formData.append('resume', this.selectedFile); // ← use correct field name
  ...
  uploadFile() {
    ...
    // console.log("📤 Uploading file to backend:", this.selectedFile);
    this.globalTextData = JSON.stringify(this.selectedFile);
    // console.log("📦 File data:", this.globalTextData);
  ...
  redirectToAnalyzeResume(): void {
    ...
    this.http.post('http://localhost:4000/api/analyze', jobData).subscribe(
      (response: any) => {
        ...
        console.log("📄 aaaaaaaaaaaaaaaaaaaaa Extracted Resume Text:", this.dataService.interview);
